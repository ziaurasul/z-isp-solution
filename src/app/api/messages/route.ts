...
export async function POST(req: NextRequest) {
  ...
  // If WhatsApp send requested and business has it enabled
  if (sendWhatsapp && business.whatsappEnabled && business.whatsappToken && customerId) {
    ...
    if (customer) {
      try {
        ...
        if (phone) {
          // Simulate WhatsApp send - in production replace with actual API call
          // await fetch(`https://graph.facebook.com/v18.0/${business.whatsappToken}/messages`, {...})
